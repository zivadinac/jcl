...
def bins_from_spike_times(spike_times, bin_len=25.6, sampling_period=0.05, dtype=np.uint16, dense_loading=True, return_mat_type=csc_matrix, limits=None, return_edges=False):
    """ Bin given spike times, each bin contains total number of spikes.

        Args:
            spike_times - list of spike times per neuron (list of iterables, pre-sorted in a non-descending order)
            bin_len - length of a bin in ms (default 1s/39.0625 = 25.6ms) or list of bin edges in ms
            ...
            limits - session limits (begin_ts, end_ts), if None (default) use first and last spike time; ignored if bin edges are provided via `bin_len`
            return_edges - whether to return bin edges; default False
        Return:
            Matrix with spike count per bin ((neuron num, time bins), `return_mat_type`)
    """
    try:
        bin_num = len(bin_len) - 1
        bin_edges = bin_len
    except TypeError:
    ...
    binned_data = return_mat_type(binned_data)
    if return_edges:
